...
describe('🤕 ❤️  Character state - schema migration', function () {
    ...
    describe('individual migration functions', function () {
        describe(`2 to latest`, function () {
            it('should work', () => {
                expect(migrations_1.migrate_to_latest(lodash_1.cloneDeep(DATA_v2), state_1.MIGRATION_HINTS_FOR_TESTS)).to.deep.equal(DATA_LATEST);
            });
        });
        describe(`1 to latest`, function () {
            it('should work', () => {
                expect(migrations_1.migrate_to_latest(lodash_1.cloneDeep(DATA_v1), state_1.MIGRATION_HINTS_FOR_TESTS)).to.deep.equal(DATA_LATEST);
            });
        });
