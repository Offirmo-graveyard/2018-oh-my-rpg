...
describe('🤕 ❤️  Character state - schema migration', function () {
    context('when the version is more recent', function () {
        it('should throw with a meaningful error', () => {
            function load() {
                migrations_1.migrate_to_latest(sec_1.getSEC(), { schema_version: 99999 });
            }
    ...
    context('when the version is up to date', function () {
        it('should return the state without change', () => {
            ...
            expect(migrations_1.migrate_to_latest(sec_1.getSEC(), lodash_1.cloneDeep(DATA_LATEST))).to.deep.equal(DATA_LATEST);
        });
    });
    context('when the version is outdated', function () {
        it('should migrate to latest version', () => {
            expect(migrations_1.migrate_to_latest(sec_1.getSEC(), lodash_1.cloneDeep(DATA_OLDEST), state_1.MIGRATION_HINTS_FOR_TESTS)).to.deep.equal(DATA_LATEST);
        });
    });
    describe('individual migration functions', function () {
        describe(`2 to latest`, function () {
            it('should work', () => {
                expect(migrations_1.migrate_to_latest(sec_1.getSEC(), lodash_1.cloneDeep(DATA_v2), state_1.MIGRATION_HINTS_FOR_TESTS)).to.deep.equal(DATA_LATEST);
            });
        });
        describe(`1 to latest`, function () {
            it('should work', () => {
                expect(migrations_1.migrate_to_latest(sec_1.getSEC(), lodash_1.cloneDeep(DATA_v1), state_1.MIGRATION_HINTS_FOR_TESTS)).to.deep.equal(DATA_LATEST);
            });
        });
        describe(`0 to latest`, function () {
            it('should work', () => {
                expect(migrations_1.migrate_to_latest(sec_1.getSEC(), lodash_1.cloneDeep(DATA_v0), state_1.MIGRATION_HINTS_FOR_TESTS)).to.deep.equal(DATA_LATEST);
            });
        });
    });
});
