...
describe('⚔ 👑 😪  The Boring RPG - schema migration', function() {
	...
	describe('individual migration functions', function () {
		...
		describe(`3 to latest`, function () {
			it('should work', () => {
				expect(migrate_to_latest(get_SEC(), cloneDeep(DATA_v3_20171108), MIGRATION_HINTS_FOR_TESTS)).to.deep.equal(DATA_LATEST)
			})
		})

		describe(`2 to latest`, function () {
			it('should work', () => {
				expect(migrate_to_latest(get_SEC(), cloneDeep(DATA_v2_20171026), MIGRATION_HINTS_FOR_TESTS)).to.deep.equal(DATA_LATEST)
			})
		})

		describe(`1 to latest`, function () {
			it('should work', () => {
				expect(migrate_to_latest(get_SEC(), cloneDeep(DATA_v1_20171025), MIGRATION_HINTS_FOR_TESTS)).to.deep.equal(DATA_LATEST)
			})
		})
